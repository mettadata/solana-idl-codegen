...
fn test_idl_parsing_performance() {
    ...
    println!("Total avg parsing time: {:.2}ms", total_time.as_micros() as f64 / 1000.0);
    println!("===============================\n");
}
...
fn test_code_generation_performance() {
    ...
    println!("Total code generation time: {:.2}ms", total_time.as_micros() as f64 / 1000.0);
    println!("====================================\n");
}
...
fn test_full_pipeline_performance() {
    ...
    for (name, path) in &test_cases {
        ...
        println!("  {} - {:.2}ms total", name, pipeline_time.as_micros() as f64 / 1000.0);
        println!("    read:       {:.2}ms", read_time.as_micros() as f64 / 1000.0);
        println!("    parse:      {:.2}ms", parse_time.as_micros() as f64 / 1000.0);
        println!("    generate:   {:.2}ms", gen_time.as_micros() as f64 / 1000.0);
    }
    
    println!("Total pipeline time: {:.2}ms", total_time.as_micros() as f64 / 1000.0);
    println!("==================================\n");
}
...
fn test_code_size_metrics() {
    ...
    for (name, path) in &test_cases {
        ...
        let total_lines = lib_lines + types_lines + accounts_lines + instructions_lines + errors_lines + events_lines;
        
        let total_bytes = generated.lib.len() + generated.types.len() + generated.accounts.len() 
            + generated.instructions.len() + generated.errors.len() + generated.events.len();
        
        println!("  {} - {} lines, {} KB", name, total_lines, total_bytes / 1024);
...
fn test_idl_complexity_metrics() {
    ...
    for (name, path) in &test_cases {
        ...
        let total_complexity = num_instructions + num_accounts + num_types + num_errors + num_events;
