...
fn load_test_idl(name: &str, path: &str) -> Idl {
    let content =
        fs::read_to_string(path).unwrap_or_else(|_| panic!("Failed to read IDL: {}", path));
    serde_json::from_str(&content).unwrap_or_else(|_| panic!("Failed to parse IDL: {}", name))
}
...
criterion_group!(benches, bench_idl_parsing, bench_code_generation);
